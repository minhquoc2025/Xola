...
def record_step_result(step_num, direction, is_correct):
    ...
    print(f"📝 Ghi nhận Bước {step_num}: hướng [{direction}] = {entry[direction]}")
...
def pick_buff_by_tier(candidates):
    ...
    for tier in BUFF_PRIORITY:
        for btn, t, desc, text in candidates:
            if t == tier:
                print(f"   🎖️ Chọn theo bậc [{tier}]: {text!r}")
                return btn, text
    if candidates:
        print(f"   ⚠️ KHÔNG xác định được bậc cho bất kỳ nút nào (kiểm tra lại wording bậc "
              f"trong BUFF_PRIORITY có khớp đúng chữ hiển thị không) -> buộc lấy nút đầu tiên.")
        return candidates[0][0], candidates[0][3]
    return None, None


def pick_buff_button_dianguc(buttons, message_text="", debug=True):
